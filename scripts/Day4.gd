...
var customers = [
	...
	["random", ["plate_bread_ham_cooked", "plate_bread_kelp_cooked"],40],
	["random", ["plate_tortilla_kelp_mayo", "plate_tortilla_meat"],40],
	["random", ["plate_tortilla_kelp_mayo", "plate_tortilla_meat"],40],

	["random", ["plate_bread_full_cooked", "coconut_umbrellablue", "coconut_umbrellapink"],40],

	["random", ["plate_tortilla_kelp_salsa", "plate_tortilla_kelpmeat", "coconut"],40],
	["loansharkshark", ["plate_bread_kelp_ham_cooked"],45, [
		"LoanShark Shark: I’m back. Sneak me the money in the food I ordered. Hurry up, or else.",
		"Hehe… Pleasure doing business with you.",
		"YOU ARE GOING TO REGRET THIS!"
	], [0],"res://sprites/customers/loansharkshark/sharkfrown.png"],

	["lobstermobster", ["plate_bread_full_cooked"],45, [
		"I’m here to collect the money your Uncle owes. Give it up now.",
		"Good, stay sharp.",
		"You better watch out."
	], [0],"res://sprites/customers/lobstermobster.png"],
...
func spawn_customer(slot, dialogue, breakpoints, orders, sprite, patience_in_seconds, cust_name):
	var customer_instance = root.spawn_customer(
			slot,
			dialogue,
			breakpoints,
	...
	customer_instance.customer_name = cust_name
	customer_instance.on_all_orders_finished.connect(_on_order_finished)
	customer_instance.on_out_of_patience.connect(_on_order_patience_run_out)
...
func next_customer(slot):
	...
	if (customer_count >= customers.size()):
		...
		if (customers[customer_count][0] == "random"):
			...
			spawn_customer(slot, customers[customer_count][3], customers[customer_count][4], customers[customer_count][1], customers[customer_count][5], customers[customer_count][2], customers[customer_count][0])
...
func end_day():
	if (!has_ended&&served_customers == customers.size()):
		...
		if (!SceneManager.paid_loanshark||!SceneManager.paid_lobstermobster):
			get_tree().change_scene_to_file("res://scenes/cutscene_mafiaending.tscn")
		else:
			get_tree().change_scene_to_file("res://scenes/level5.tscn") # what do you want
